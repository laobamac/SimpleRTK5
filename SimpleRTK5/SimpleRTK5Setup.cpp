    ...
    if (params) {
        ...
        if (fbAddr) {
            ...
            if (fbAddr->getLength()) {
                ...
                IOLog("SimpleRTK5: Fallback MAC: %2.2x:%2.2x:%2.2x:%2.2x:%2.2x:%2.2x\n",
...
bool SimpleRTK5::initEventSources(IOService *provider)
{
    ...
    if (txQueue == NULL) {
        IOLog("SimpleRTK5: Failed to get output queue.\n");
        goto done;
    }
    txQueue->retain();
...
bool SimpleRTK5::setupRxResources()
{
    ...
    if (!rxBufDesc) {
        IOLog("SimpleRTK5: Couldn't alloc rxBufDesc.\n");
        goto error_buff;
    }
    if (rxBufDesc->prepare() != kIOReturnSuccess) {
        IOLog("SimpleRTK5: rxBufDesc->prepare() failed.\n");
        goto error_prep;
    }
...
bool SimpleRTK5::setupTxResources()
{
    ...
    if (!txBufDesc) {
        IOLog("SimpleRTK5: Couldn't alloc txBufDesc.\n");
        goto error_buff;
    }
    if (txBufDesc->prepare() != kIOReturnSuccess) {
        IOLog("SimpleRTK5: txBufDesc->prepare() failed.\n");
        goto error_prep;
    }
    ...
    if (!txDescDmaCmd) {
        IOLog("SimpleRTK5: Couldn't alloc txDescDmaCmd.\n");
        goto error_dma;
    }
...
bool SimpleRTK5::setupStatResources()
{
    IODMACommand::Segment64 seg;
    UInt64 offset = 0;
    UInt32 numSegs = 1;
    ...
    if (statBufDesc->prepare() != kIOReturnSuccess) {
        IOLog("SimpleRTK5: statBufDesc->prepare() failed.\n");
        goto error_prep;
    }
    ...
    if (!statDescDmaCmd) {
        IOLog("SimpleRTK5: Couldn't alloc statDescDmaCmd.\n");
        goto error_dma;
    }
...
void SimpleRTK5::freeRxResources()
{
    UInt32 i;
