...
//  SimpleRTK5
//
//  Created by laobamac on 2025/10/4.
//
...
bool SimpleRTK5::initPCIConfigSpace(IOPCIDevice *provider)
{
    ...
    if (provider->extendedFindPCICapability(kIOPCIPCIExpressCapability, &pcieCapOffset)) {
        ...
        DebugLog("SimpleRTK5: PCIe link capabilities: 0x%08x, link control: 0x%04x.\n", pcieLinkCap, pcieLinkCtl);
...
bool SimpleRTK5::initRTL8126()
{
    ...
    if (identifyChip()) {
        IOLog("SimpleRTK5: Unsupported chip found. Aborting...\n");
        goto done;
    }
    ...
    if (is_valid_ether_addr((UInt8 *)macAddr)) {
        IOLog("SimpleRTK5: Get macAddr %02x:%02x:%02x:%02x:%02x:%02x\n",
              macAddr[0], macAddr[1], macAddr[2], macAddr[3], macAddr[4], macAddr[5]);
        rtl8126_rar_set(tp, macAddr);
    } else {
        IOLog("SimpleRTK5: Using fallback MAC.\n");
    ...
    IOLog("SimpleRTK5: %s: (Chipset %d), %2.2x:%2.2x:%2.2x:%2.2x:%2.2x:%2.2x\n",
...
#ifdef DEBUG
    if (wolCapable)
        IOLog("SimpleRTK5: Device is WoL capable.\n");
#endif
...
{
    UInt8 deviceControl;
    DebugLog("SimpleRTK5: setOffset79() ===>\n");
...
UInt16 SimpleRTK5::getEEEMode()
{
    ...
    if (eeeCap) {
        ...
        ena &= BIT_1 | BIT_0;
        DebugLog("SimpleRTK5: EEE enabled: %u\n", ena);
...
void SimpleRTK5::powerDownPLL()
{
    ...
    if (tp->wol_enabled == WOL_ENABLED || tp->EnableKCPOffload) {
        ...
        if (tp->mcfg == CFG_METHOD_2 || tp->mcfg == CFG_METHOD_3 || tp->mcfg == CFG_METHOD_1) {
            int ctrl_2500;
            ctrl_2500 = rtl8126_mdio_direct_read_phy_ocp(tp, 0xA5D4);
            ctrl_2500 &= ~(RTK_ADVERTISE_2500FULL | RTK_ADVERTISE_5000FULL);
            rtl8126_mdio_direct_write_phy_ocp(tp, 0xA5D4, ctrl_2500);
...
void SimpleRTK5::configPhyHardware8126a2()
{
    ...
    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xA438, 0xFF00, 0xED00);

    rtl8126_mdio_direct_write_phy_ocp(tp, 0xA436, 0x80CD);
    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xA438, 0xFF00, 0x1000);
    rtl8126_mdio_direct_write_phy_ocp(tp, 0xA436, 0x80D1);
    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xA438, 0xFF00, 0xC800);
    ...
    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xA438, 0xFF00, 0x4700);
    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xA80C, BIT_7 | BIT_6, BIT_7);
    ...
    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xB87E, 0xFF00, 0x1100);
    rtl8126_set_eth_phy_ocp_bit(tp, 0xACF8, (BIT_3 | BIT_2));
    rtl8126_mdio_direct_write_phy_ocp(tp, 0xA436, 0x8183);
    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xA438, 0xFF00, 0x5900);
    ...
    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xB87E, 0xFF00, 0x2F00);
    rtl8126_mdio_direct_write_phy_ocp(tp, 0xB87C, 0x83F2);
    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xB87E, 0xFF00, 0x0800);
    rtl8126_set_eth_phy_ocp_bit(tp, 0xADA0, BIT_1);

    rtl8126_mdio_direct_write_phy_ocp(tp, 0xB87C, 0x80F3);
    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xB87E, 0xFF00, 0x9900);
    rtl8126_mdio_direct_write_phy_ocp(tp, 0xB87C, 0x8126);
    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xB87E, 0xFF00, 0xC100);
    ...
    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xB87E, 0xFF00, 0xE600);
    rtl8126_mdio_direct_write_phy_ocp(tp, 0xB87C, 0x862C);
    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xB87E, 0xFF00, 0x1200);
    ...
    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xB87E, 0xFF00, 0x5A00);
    ...
    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xB87E, 0xFF00, 0x3000);
    rtl8126_mdio_direct_write_phy_ocp(tp, 0xB87C, 0x8078);
    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xB87E, 0xFF00, 0x3000);
    rtl8126_mdio_direct_write_phy_ocp(tp, 0xB87C, 0x8031);
    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xB87E, 0xFF00, 0x3300);
    rtl8126_mdio_direct_write_phy_ocp(tp, 0xB87C, 0x8073);
    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xB87E, 0xFF00, 0x3300);

    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xAE06, 0xFC00, 0x7C00);
    ...
    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xA438, 0xFF00, 0x0A00);
    ...
    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xBD96, 0x1F00, 0x1000);
    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xBF1C, 0x0007, 0x0007);
    rtl8126_clear_eth_phy_ocp_bit(tp, 0xBFBE, BIT_15);
    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xBF40, 0x0380, 0x0280);
    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xBF90, BIT_7, (BIT_6 | BIT_5));
    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xBF90, BIT_4, BIT_3 | BIT_2);

    rtl8126_clear_phy_mcu_patch_request(tp);

    rtl8126_mdio_direct_write_phy_ocp(tp, 0xA436, 0x843B);
    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xA438, 0xFF00, 0x2000);
    ...
    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xB87E, 0xFF00, 0x0000);
    rtl8126_mdio_direct_write_phy_ocp(tp, 0xB87C, 0x8FFF);
    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xB87E, 0xFF00, 0x7F00);
    rtl8126_mdio_direct_write_phy_ocp(tp, 0xB87C, 0x8FFB);
    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xB87E, 0xFF00, 0x0100);
    ...
    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xB87E, 0xFF00, 0x0400);
    ...
    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0XAC3C, BIT_15 | BIT_14 | BIT_12, BIT_13);
    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xAC42, BIT_9, BIT_8 | BIT_7 | BIT_6);
    rtl8126_clear_eth_phy_ocp_bit(tp, 0xAC3E, BIT_15 | BIT_14 | BIT_13);
    rtl8126_clear_eth_phy_ocp_bit(tp, 0xAC42, BIT_5 | BIT_4 | BIT_3);
    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xAC42, BIT_1, BIT_2 | BIT_0);
    ...
    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xB87E, 0xFF00, 0x0B00);
    ...
    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xB87E, 0xFF00, 0x0700);
    rtl8126_mdio_direct_write_phy_ocp(tp, 0xB87C, 0x811A);
    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xB87E, 0xFF00, 0x0700);
    rtl8126_mdio_direct_write_phy_ocp(tp, 0xB87C, 0x811C);
    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xB87E, 0xFF00, 0x0500);
    ...
    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xADE8, 0xFFC0, 0x1400);
    rtl8126_mdio_direct_write_phy_ocp(tp, 0xB87C, 0x864B);
    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xB87E, 0xFF00, 0x9D00);
    ...
    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xB87E, 0xFF00, 0x0C00);
    ...
    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xB54C, 0xFFC0, 0x3700);
...
void SimpleRTK5::configPhyHardware8126a3()
{
    ...
    rtl8126_set_eth_phy_ocp_bit(tp, 0xAD94, BIT_5);
    rtl8126_set_eth_phy_ocp_bit(tp, 0xADA0, BIT_1);
    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xAE06, BIT_15 | BIT_14 | BIT_13 | BIT_12 | BIT_11 | BIT_10, BIT_14 | BIT_13 | BIT_12 | BIT_11 | BIT_10);
    ...
    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xB87E, 0xFF00, 0x0200);
    rtl8126_mdio_direct_write_phy_ocp(tp, 0xB87C, 0x8FFF);
    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xB87E, 0xFF00, 0x0400);

    rtl8126_mdio_direct_write_phy_ocp(tp, 0xA436, 0x8018);
    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xA438, 0xFF00, 0x7700);
    ...
    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xA438, 0xFF00, 0x2900);
    ...
    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xADE8, BIT_15 | BIT_14 | BIT_13 | BIT_12 | BIT_11 | BIT_10 | BIT_9 | BIT_8 | BIT_7 | BIT_6, BIT_12 | BIT_10);
    rtl8126_mdio_direct_write_phy_ocp(tp, 0xB87C, 0x864B);
    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xB87E, 0xFF00, 0x9D00);
    rtl8126_mdio_direct_write_phy_ocp(tp, 0xB87C, 0x862C);
    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xB87E, 0xFF00, 0x1200);
    ...
    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xB87E, 0xFF00, 0x5D00);
    rtl8126_mdio_direct_write_phy_ocp(tp, 0xB87C, 0x807C);
    rtl8126_clear_and_set_eth_phy_ocp_bit(tp, 0xB87E, 0xFF00, 0x5000);
